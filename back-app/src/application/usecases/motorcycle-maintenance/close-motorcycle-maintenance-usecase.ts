...
export class CloseMotorcycleMaintenanceUsecase {
	...
	public async execute(maintenanceId: string) {
		...
		try {
			const promises = motorcycleMaintenance.usedSpareParts.map(
				async(maintenanceSparePart) => {
					const sparePartEntity = await this.sparePartRepository.findById(maintenanceSparePart.sparePartId);
		...
		motorcycleMaintenance.totalMaintenancePrice = calculatedTotalSparePartsPrice + motorcycleMaintenance.laborPrice;
